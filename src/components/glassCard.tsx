...
export default function GlassCard({ children }: Props) {
    ...
    return (
        <div ref={cardRef} onMouseMove={handleMouseMove} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}
            className=" relative w-full h-32 flex  rounded-sm  border border-white/10 bg-gradient-to-br from-white/20 to-white/5 backdrop-blur-[2px] shadow-[0_8px_30px_rgba(0,0,0,0.35)] 
            overflow-hidden transition-[background] duration-300 p-3"
            style={{
                background: `
          radial-gradient(
            70px circle at var(--x, 50%) var(--y, 50%),
            rgba(255,255,255,calc(0.35 * var(--glow, 0))),
            transparent 60%
          ),
